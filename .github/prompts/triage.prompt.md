...
1. **Three-layer scan:**
   ...
   b. File listing -- complete inventory of all report files:
   ...
   c. Cross-reference both lists. Flag any discrepancies:
      - Files in git status but not in the listing (shouldn't happen -- defensive)
...
3. **Classify files:**
   ...
   - Unchanged reports (in listing but not in git status): skip -- already processed.
...
5. **If CI fails:** diagnose and fix (same logic as `/fix-ci`, max 3 iterations).
...
- **Exhaustive discovery.** Use all three scan layers (git status, file listing, cross-reference). Never assume how many reports exist. Present the full count before processing.
