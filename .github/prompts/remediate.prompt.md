...
If `${input:reportPath}` is provided, use it as the report path or wave
selector (e.g., `wave=2` to resume at Wave 2). Otherwise, auto-detect the
report at `docs/agents/pre-launch-report.md`. If no report exists, suggest
...
2. **Extract EVERY finding** -- all 5 severity tiers: launch-blocker,
   high, medium, low, strategic. No filtering by severity -- 100%
   coverage.
...
3. **Group related findings into work units:**
   ...
   is the only exception to 100% coverage: low and strategic items require
   human architectural judgment that agents cannot reliably provide.

4. **Detect the integration branch:**
   - Check AGENTS.md or git config for the documented integration branch.
...
2. **For each Wave 1 work unit, create a worktree and implement the
   fix:**

   ```bash
   git worktree add ../remediate-<slug> -b remediate/<slug>
   ```
   ...
   a. Read the GitHub issue and all source files in the ownership set.
   ...
   e. Run verification sequentially via terminal:
      `$TEST_CMD; $TYPECHECK_CMD; $LINT_CMD`

   f. Run `/quality-review` on changed files.

   g. Run verification again (in case `/quality-review` introduced changes).
...
3. **Monitor Wave 1 agent progress.** As agents complete, log their
   ...
   (Rule #44 coverage is satisfied by the open issue).
...
4. **Merge PRs sequentially** to the integration branch. For each PR:
   ...
   b. Pull: `git pull`
   c. Run full verification via terminal.
   d. If tests break, fix before proceeding to the next merge.
   e. Close the corresponding GitHub issue after successful merge.

5. **Run `/quality-review`** on the full integrated Wave 1 result.

6. **Run Wave 1 final verification** via terminal.
...
4. **Run `/quality-review`** on the full integrated Wave 2 result.
...
- [ ] /quality-review final pass complete for waves that ran
...
- **100% coverage.** Process EVERY finding -- all 5 severity tiers.
  ...
  human architectural judgment -- the one documented exception). Every
  finding still gets an issue.
...
- **User can defer waves.** After any wave, user may ship and schedule
  the next wave separately. Pass `wave=N` in the input to resume.
...
- **Recommendation is a hypothesis** (Rule #55). Verify the finding's
  assumptions in real code before implementing (worktree step b). The guard
  test asserts the invariant the fix could break, not just the finding's
  symptom. A recommendation that fails verification or trades away a
  correctness, security, or UX invariant **halts** -- escalate to the human,
  never implement it literally and never silently substitute your own fix.
...
- **Branch verification before every commit.** Run
  `git branch --show-current` and verify the result.
- **/quality-review twice.** Once per agent (after their fix), once on
  the integrated result per wave.
