...
If `${input:reportPath}` is provided, use it as the report path. Otherwise, auto-detect the report at `docs/agents/pre-launch-report.md`. If no report exists, suggest running `/pre-launch` first and **STOP.**
...
4. **Detect the integration branch:**
   - Check AGENTS.md or git config for the documented default branch.
...
2. **For each work unit, create a worktree and implement the fix:**
...
3. **Monitor progress.** As agents complete, log their status (pass/fail, tests added, files modified).
...
5. **Run `/quality-review`** on the full integrated result.

6. **Run final verification** via terminal.
...
- [ ] Build succeeds
- [ ] CI green
- [ ] /quality-review final pass complete
...
- **TDD mandatory.** Write a failing test before implementing each fix. The only exception is non-testable work (documentation, configuration, CI changes).
- **Agents do NOT push.** Only the orchestrator pushes. Worktree agents commit locally, orchestrator batch-pushes all branches.
...
- **Branch verification before every commit.** Run `git branch --show-current` and verify the result before committing.
- **/quality-review twice.** Once per agent (after their fix), once on the integrated result.
