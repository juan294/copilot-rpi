...
This is a copilot-rpi blueprint template. Copy it into a target project and adapt it. It is
...
## Where This Fits in copilot-rpi
...
machinery copilot-rpi already ships — it plugs an evidence gate in front of the tag step and
delegates to the existing prompts:
...
- **`/release`** stays the single tagging and versioning authority. E2E Pro's release procedure
  ...
  divergent tag process. This is decision D01 applied to copilot-rpi itself.
...
- The machine-readable requiredness contract and fail-closed analyzer (Wave A2) follow the same
  pattern as this blueprint's repo-invariant scripts (`verify-counts.sh`, `verify-version.sh`,
  `verify-prompts.sh`) and their `BLOCKED/WHY/FIX` convention. Reuse that convention rather than
  inventing a second gate style. Copilot has no edit-time hook mechanism, so CI is where these
  gates run.
- **`/explore-release`** (blueprint prompt) runs Wave B charters.
...
.github/prompts/explore-release.prompt.md
```
...
| Agent prompt directory | `<AGENT_PROMPT_DIRECTORY>` (Copilot: `.github/prompts/`) |
...
This wave is mandatory before calling the system E2E Pro. In copilot-rpi terms, Wave A is the floor
...
Tagging before steps 1–8 is forbidden. In copilot-rpi, steps 8-9 are performed by `/release` — this
...
encode. In copilot-rpi, this wave is executed by the `/explore-release` command.
...
instead of expanding it indefinitely. In copilot-rpi, the tag/publish step (8) is executed by
