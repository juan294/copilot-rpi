---
mode: agent
description: "Process all overnight agent reports, GitHub Security & Quality Alerts, and the Dependabot PR queue -- discover, analyze, fix everything, commit, merge"
---
Model tier: **sonnet** — invoke this prompt in a Sonnet session.

# Triage Agent Reports

Process all overnight agent reports, GitHub Security & Quality Alerts, and the Dependabot PR queue. Discovers every report using timestamp-based discovery, checks for agent failures, queries GitHub-native alert surfaces (code scanning/CodeQL, Dependabot security, secret scanning), scans open Dependabot PRs (Rule #45), synthesizes findings, proposes an action plan, implements all fixes, and merges the Dependabot PRs that are safe to auto-merge. Report commit policy depends on repo visibility: public repos keep reports local, private repos commit them as historical artifacts (Rule #42).

## Input

If `${input:reportPath}` is provided, process only the specified report path(s). Otherwise, auto-discover all new/modified reports in `docs/agents/`. If no reports found and no agent failures detected, report "all clear" and **STOP.**

## Step 1: Discovery

Find EVERY report and agent failure. No assumptions about which agents ran or how many reports exist. Discovery uses file timestamps, not git status.
...
1. **Timestamp-based scan:**
   ...
   c. If NO marker exists (first run) -- process ALL reports:
      ...
      ```bash
      ls docs/agents/*-report.md 2>/dev/null
      ```

   d. Full inventory (for cross-reference and completeness):

      ```bash
      ls docs/agents/*.md 2>/dev/null
      ```
...
3. **Check for open Dependabot PRs (Rule #45):**
...
4. **Check GitHub Security & Quality Alerts (critical):**
   ...
   - **API/query failure:** if any GitHub alert query fails, returns 403/404,
     ...
     (A 403 on code scanning usually means GHAS is off -- see Rule #52.)
...
3. **Leanness report handling:** If a discovered report is
   ...
   Leanness items still follow Rule #37 after user approval: fix all extracted
...
7. **Draft the action plan:**
   ...
   Group action items by report. Include ALL items -- fix everything (Rule #37). For each item: what to do, which files, expected outcome.
...
2. **Run verification sequentially** via terminal.

3. **Run `/quality-review`** on all changed files.

4. **Run verification again** (in case `/quality-review` introduced changes).

## Step 4: Commit & Push

Commit policy depends on repo visibility (Rule #42). Determine visibility before staging:
...
After the triage commit is pushed and green, process the Dependabot PRs identified in Step 1.3 (Rule #45). These are independent commits from the triage code fixes -- handle them last so a flaky dependency PR doesn't block triage.
...
- **Exhaustive discovery.** Use timestamp-based scan with `.last-triage` marker (Rule #43). Never assume how many reports exist. Present the full count before processing.
- **Report commit policy is visibility-conditional (Rule #42).** Public repos: reports stay local, only code fixes are committed (`docs/agents/`, `logs/`, `scripts/agents/` gitignored). Private repos: reports are committed alongside code fixes as historical artifacts.
- **GitHub alert coverage is mandatory.** Every triage run must query and report GitHub code scanning alerts (including CodeQL and quality warnings), Dependabot security alerts, and secret scanning alerts. Do not rely only on local agent reports. If a query fails or alerts appear disabled unexpectedly, report that as a YELLOW/RED triage finding and action item. (Querying alerts is always safe; creating a code scanner requires GHAS -- Rule #52.)
- **Process Dependabot PRs (Rule #45).** Triage scans for open Dependabot PRs and merges what it can: patch + minor with green CI auto-merge, majors defer for human review, obvious CI failures get one fix attempt. Dependabot processing happens last so it can't block triage code fixes.
...
- **Fix everything (Rule #37).** Categorize findings by severity, but implement 100% of action items. No deferring. No "nothing urgent." `leanness-report.md` is actionable: extract and implement every concrete recommendation after the user approves the action plan.
...
- **Branch verification before every commit.** Run `git branch --show-current` first.
