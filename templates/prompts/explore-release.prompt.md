---
mode: agent
description: "Run fresh-context exploratory release charters against a fixed candidate (E2E Pro Wave B)"
---
Model tier: **opus** — invoke this prompt in an Opus session for the orchestrator.
Charter agents run as parallel `copilot -p` processes or `@copilot` issues; tier
them by cost (sonnet is usually sufficient for a single charter's execution).
...
decision detail. This prompt is the executable protocol.

## Input

`${input:lastReleaseRef}` — the previous release tag or ref.
`${input:candidateSha}` — the fixed candidate SHA or artifact digest.

If either is omitted, infer `lastReleaseRef` from the last release tag and
`candidateSha` from the fixed candidate under release. Confirm both with the user
before spawning agents — a charter run against the wrong candidate is wasted.
...
2. Compute the change surface via #tool:terminal:

   ```bash
   git log --oneline <lastReleaseRef>..<candidateSha>
   git diff --stat <lastReleaseRef>..<candidateSha>
   ```
...
Spawn one agent per charter — a separate `copilot -p` process or an `@copilot`
issue. Each agent MUST be a fresh context that:
...
Give every spawned agent a single-sentence task, an explicit stop condition, and a
wall-clock budget (Rule #48). Kill overruns rather than assuming progress.
...
Do not tag or release from this prompt. Hand the evidence to `/release`, which
gates on it.
...
- A charter finding is a diagnosis, not a work order (Rule #55) — when it feeds
  `/remediate`, its recommendation is verified against real code before anyone
  implements it.
