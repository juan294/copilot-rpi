---
mode: agent
description: "A disciplined procedure for finding root causes of novel bugs -- reproduce, isolate, hypothesize, fix, verify"
---
Model tier: **sonnet** — invoke this prompt in a Sonnet session.

# Systematic Debugging

Find the root cause of a non-obvious bug: ${input:bug}

Use this when a bug is non-obvious, a fix didn't hold, or you've tried the same
thing twice without progress. For known tool/git/CI failures, check
`patterns/quick-reference.md` and `patterns/agent-errors.md` first — this
procedure is for novel bugs. When stuck, stop guessing and run the loop. Random
edits that "might fix it" waste turns and mask the real cause.
...
- Same approach twice with no new information -> stop, change technique, or
  escalate. Repeating a failed action is a known error pattern.
